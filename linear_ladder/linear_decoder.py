...
class LINEARDecoder(nn.Module):
    def __init__(self, d_in, d_out, use_cuda):
        ...
        if self.use_cuda:
            self.a1 = Parameter(torch.zeros(1, d_in).cuda(), requires_grad=True)
            self.a2 = Parameter(torch.ones(1, d_in).cuda(), requires_grad=True)
            self.a3 = Parameter(torch.zeros(1, d_in).cuda(), requires_grad=True)
            self.a4 = Parameter(torch.zeros(1, d_in).cuda(), requires_grad=True)
            self.a5 = Parameter(torch.zeros(1, d_in).cuda(), requires_grad=True)

            self.a6 = Parameter(torch.zeros(1, d_in).cuda(), requires_grad=True)
            self.a7 = Parameter(torch.ones(1, d_in).cuda(), requires_grad=True)
            self.a8 = Parameter(torch.zeros(1, d_in).cuda(), requires_grad=True)
            self.a9 = Parameter(torch.zeros(1, d_in).cuda(), requires_grad=True)
            self.a10 = Parameter(-1 * torch.ones(1, d_in).cuda(), requires_grad=True)
        else:
            self.a1 = Parameter(torch.zeros(1, d_in), requires_grad=True)
            self.a2 = Parameter(torch.ones(1, d_in), requires_grad=True)
            self.a3 = Parameter(torch.zeros(1, d_in), requires_grad=True)
            self.a4 = Parameter(torch.zeros(1, d_in), requires_grad=True)
            self.a5 = Parameter(torch.zeros(1, d_in), requires_grad=True)

            self.a6 = Parameter(torch.zeros(1, d_in), requires_grad=True)
            self.a7 = Parameter(torch.ones(1, d_in), requires_grad=True)
            self.a8 = Parameter(torch.zeros(1, d_in), requires_grad=True)
            self.a9 = Parameter(torch.zeros(1, d_in), requires_grad=True)
            self.a10 = Parameter(-1 * torch.ones(1, d_in), requires_grad=True)
        ...
        # BN MODULE
            self.bn = torch.nn.BatchNorm1d(d_out, affine=False)
            if self.use_cuda:
                self.bn.cuda()
    ...
    def g(self, tilde_z, u):
        if self.use_cuda:
            ones = torch.ones(tilde_z.size()[0], 1).cuda()
        else:
            ones = torch.ones(tilde_z.size()[0], 1)
        b_a1 = ones*self.a1
        b_a2 = ones*self.a2
        b_a3 = ones*self.a3
        b_a4 = ones*self.a4
...
class LINEARStackedDecoders(nn.Module):
    def __init__(self, d_in, d_decoders, origin_size, use_cuda):
        super(LINEARStackedDecoders, self).__init__()
        self.use_cuda = use_cuda
        ...
        for i in range(n_decoders):
            ...
            decoder = LINEARDecoder(d_input, d_output, use_cuda)
            self.decoders_ref.append(decoder_ref)
            self.decoders.add_module(decoder_ref, decoder)
        self.bottom_decoder = LINEARDecoder(origin_size, None, use_cuda)
    ...
    def bn_hat_z_layers(self, hat_z_layers, z_pre_layers):
        ...
        for i, (hat_z, z_pre) in enumerate(zip(hat_z_layers, z_pre_layers)):
            ones = torch.ones(z_pre.size()[0], 1)
